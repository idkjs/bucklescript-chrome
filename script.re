...
type ctype =
  | CObject(
      string, /* name */
      list((string, string)),
    ) /* list of fields consisting name and type */
  | CBool
  | CString
  | CFn(
      string, /* param type */
...
type cmethod =
  | CMethod(string, list((string, ctype))); /* name of method, and args with names */
type cevent =
  | CEvent(string); /* name of event */
type cmodule =
  /* a chrome module, has the implicit chrome. prefix */
  | CModule(
      string, /* name of module   */
      list(ctype), /* declared types   */
      list(cmethod), /* declared methods */
      list(cevent),
    ); /* declared events  */
...
let codegen_fields = fields => {
  let c = ((name, ctype)) =>
    space_between([kw_method, name, kw_colon, ctype]);
  line_between(List.map(c, fields));
};

let codegen_type = ctype =>
  switch (ctype) {
  | CBool
  | CString
  | CFn(_) => "" /* primitive don't need to be codegen-ed */
  | CObject(name, fields) =>
    let uncap_name = String.uncapitalize_ascii(name);
    let params =
      List.map(((param, ctype)) => "?" ++ param ++ ":" ++ ctype, fields);
    let fn_ty = String.concat(" -> ", params @ [kw_unit, uncap_name]);
    line_between([
      space_between([
        kw_class,
        kw_type,
...
let gen_fn_sig = (args, ret) => {
  ...
  String.concat(
    " -> ",
    List.map(collect_args, args) @ [kw_unit, string_of_ctype(ret)],
  );
};
 ...
 * */
let codegen_param = (mtd, (name, ctype)) =>
  switch (ctype) {
  | CBool
  | CString
  | CFn(_) => ""
  | CObject(ty, props) =>
    ...
    let p = mtd ++ String.capitalize_ascii(name);
    line_between([
...
let codegen_method = (module_name, CMethod(method_name, params)) => {
  let collect_params = ((param, ctype)) =>
    switch (ctype) {
    | CBool
    | CString => string_of_ctype(ctype)
    | CFn(_) => "(" ++ string_of_ctype(ctype) ++ ")"
    | CObject(_) => method_name ++ String.capitalize_ascii(param)
    };

  let method_sig =
    String.concat(" -> ", List.map(collect_params, params) @ ["unit"]);
  let params_type =
    line_between(List.map(codegen_param(method_name), params));
  line_between([
    params_type,
    space_between([
      kw_external,
...
let codegen_module = (CModule(name, types, methods, events)) => {
  let module_name = String.capitalize_ascii(name);
  let type_code = List.map(codegen_type, types);
  let method_code = List.map(codegen_method(name), methods);
  line_between([
