module Identity = {
  ...
  [@bs.obj]
  external mkRmCachedTokenOptions:
    (~token: string, unit) => rmCachedTokenOptions;
  ...
  [@bs.obj]
  external mkWebFlowOptions:
    (~url: string, ~interactive: bool=?, unit) => webAuthFlowOptions;

  [@bs.scope ("chrome", "identity")] [@bs.val]
  external launchWebAuthFlow:
    (webAuthFlowOptions, (Js.null(string), unit) => 'a) => unit =
    "launchWebAuthFlow";

  /* chrome.identity.getRedirectURL */

  [@bs.scope ("chrome", "identity")] [@bs.val]
  external getRedirectURL: Js.null(string) => string = "getRedirectURL";

  /* chrome.identity.onSignInChanged.addListener */

  module OnSignInChanged = {
    [@bs.scope ("chrome", "identity", "onSignInChanged")] [@bs.val]
    external addListener: ((accountInfo, bool) => 'a) => unit = "addListener";
  };
};
